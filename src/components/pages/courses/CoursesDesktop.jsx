...
import { AudioData, DesktopImgData } from "../../../utils/PracticalCoursesData";
...
export default function CoursesDesktop() {
  ...
  const totalImages = DesktopImgData.length; // Total de imágenes

  const handleLogout = () => {
    dispatch(logoutUser());
  };

  const paginatedDataDesktop = DesktopImgData.slice(
  ...
  const handleNextImageDesktop = () => {
    if (currentImageIndex < paginatedDataDesktop.length - 1) {
      setCurrentImageIndex(currentImageIndex + 1);
    } else if (currentPage < Math.ceil(DesktopImgData.length / pageSize)) {
      setCurrentPage(currentPage + 1);
  ...
  const disableNextButton =
    currentPage === Math.ceil(DesktopImgData.length / pageSize) &&
    currentImageIndex >= paginatedDataDesktop.length - 1;
            ...
            <section className="text-center">
              ...
              <div className="">
                ...
                <div className="mb-4 mm">
                  {paginatedDataDesktop.map((image) => {
                ...
                <div className="text-center mb-4">
                  <span className="font-montserrat-bold text-lg">
                    {currentImageIndex + 1 +
                      (currentPage - 1) * pageSize}/{totalImages}
                  </span>
                </div>
              </div>

              {/* Botones de navegación y contador de páginas */}
              <div className="space-x-7 flex flex-row mx-auto mt-4 items-center justify-center md:mt-5">
                <Link
                  to="/"
                ...
                >
                  {disableNextButton && <RightButtonDisabled />}
                  {!disableNextButton && <RightButton />}
                </button>
              </div>
            </section>
          </div>
